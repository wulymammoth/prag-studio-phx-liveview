defmodule LiveViewStudioWeb.SearchLive do
  ...
  def mount(_arams, _sesh, socket) do
    socket = assign(socket, zip: nil, loading: false, stores: [])
    {:ok, socket}
  end
  ...
  def handle_event("zip-search", %{"zip" => zip}, socket = %{assigns: %{loading: loading}}) do
    ...
    {:noreply, assign(socket, loading: !loading, stores: [], zip: zip)}
  end

  def handle_info({:zip_search, zip}, socket = %{assigns: %{loading: loading}}) do
    case Stores.search_by_zip(zip) do
      ...
      stores ->
        {:noreply, assign(socket, loading: !loading, stores: stores)}
    end
  end
end
